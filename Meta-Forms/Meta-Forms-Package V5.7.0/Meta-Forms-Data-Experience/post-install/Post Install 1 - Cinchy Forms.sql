...
a.[Application URL] = CASE WHEN a.[Full Name] = 'Cinchy Forms.Editor' THEN @MetaFormsURL
					  ELSE REPLACE(REPLACE(LOWER(a.[Application URL]), 'https://pilot.cinchy.net/dxdev/cinchy', LOWER(@CinchyURL)),'2',CAST(e.[Cinchy Id] AS VARCHAR(100)))
                      END
...
tc.[Calculated Column Expression] = 
  REPLACE(REPLACE(
    tc.[Calculated Column Expression],'/dxdev/cinchy',p.[Path]),'appId=2',CONCAT('appId=',CAST(e.[Cinchy Id] AS VARCHAR(100))))
...
s.[Json] =
  REPLACE(REPLACE(
    s.[Json],'/dxdev/cinchy',p.[Path]),'appId=2',CONCAT('appId=',CAST(e.[Cinchy Id] AS VARCHAR(100))))
